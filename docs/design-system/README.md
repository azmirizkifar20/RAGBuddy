...
- Sticky 3.5rem topbar: menu button (mobile), breadcrumb derived from the route, add-project button, `WindowControls` (Electron only — see below). The theme toggle lives in the sidebar footer, not the topbar. Uses `bg-sidebar`/`border-sidebar-border` (not `bg-background`/`border`) so its bottom border reads as one continuous line with the sidebar's own header row instead of two abutting different-brightness regions — `--sidebar` and `--background` are deliberately different shades (dark mode especially), and matching backgrounds only at this one shared seam avoids a visible step where the two headers meet (fixed 2026-08-11; see [../issue/2026-08-11_header-border-misalignment.md](../issue/2026-08-11_header-border-misalignment.md)).
- The topbar is the Electron shell's drag region (`app-drag-region` class, a no-op outside Electron): every interactive child inside it (menu button, breadcrumb links, add-project button, `WindowControls`) carries `app-no-drag-region` so it stays clickable rather than dragging the window.
...
| `UploadPanel` | `components/upload-panel.tsx` | Drag-and-drop uploader plus the uploaded-document list. While uploading: the drop zone's copy is replaced by the current stage (latest SSE `log` line) and a 1.5px progress bar — a real animated-width percentage once `progress` events arrive (the embedding stage), an indeterminate sliding-segment animation (`.progress-bar-indeterminate`) for stages with no total (extracting, chunking, saving). |
...
| `FormattedChatMessage` | `components/formatted-chat-message.tsx` | Custom markdown renderer for chat replies — paragraphs, lists, inline code badges, striped scrollable tables, collapsible code blocks (>20 lines) with a copy button. Body text is `text-sm`, matching user bubbles. A fenced ` ```mermaid ` block renders via `MermaidBlock` instead of as code. |
| `MermaidBlock` | `components/mermaid-block.tsx` | Renders one Mermaid diagram from an LLM-generated chat reply — never trusted to be valid syntax. `mermaid.render()` doesn't reliably *throw* on bad input; for many parse errors it resolves normally with its own internal "error" diagram (an SVG containing `error-icon`/`error-text` classes reading "Syntax error in text") — detected via those class names and treated the same as a thrown error, showing a compact "Invalid diagram" box (the raw code + a hint) instead of mermaid's own giant red error graphic. Renders into an off-screen-but-attached container (`position: fixed; top/left: -10000px`, needed because a fully detached element breaks flowchart's layout measurement) that is always removed in a `finally` — mermaid's own cleanup step is skipped on a parse-error throw, which otherwise permanently orphans that scratch element under `<body>`, outside every component tree, wherever the browser happens to lay out a trailing block-level sibling (this is what previously showed up "below the chat input" instead of inside the message). Re-renders on every light/dark theme change via a shared `MutationObserver`-backed store. See [../issue/2026-08-11_mermaid-error-leaks-into-body.md](../issue/2026-08-11_mermaid-error-leaks-into-body.md). |
| `Settings` page | `pages/settings.tsx` | Read-only `dl` groups (Vector store, Paths) bookend two `CredentialsManager` sections (embedding, chat) and a Qdrant "Danger zone". |
| `CredentialsManager` | `components/credentials-manager.tsx` | A list-of-cards editor for saved provider credentials (name, provider `Select`, base URL, write-only API-key `Input`, comma-separated models rendered as clickable chips) — mounted once per `kind` (`embedding`/`chat`) against its own independent list. Each card: inline edit (pencil) and remove (trash, no confirm), model chips activate that (credential, model) pair. The Add-credential form has its own inline Test-connection check (success/error shown inline next to the fields, not a toast, since the result needs to stay visible next to what produced it). The embedding instance wraps every non-active model chip in an `AlertDialog` confirming the re-ingest requirement before switching — chat has no such warning. |
| `QdrantDangerZone` | `pages/settings.tsx` | The one deliberately destructive control in the app: drop-collection, gated behind an `AlertDialog` whose confirm button stays disabled until the user types the collection's exact name into a text field — stricter than `DeleteConfirmModal`'s single click, since this wipes every project at once instead of one registry entry. |
...
| `WindowControls` | `components/layout/window-controls.tsx` | Custom minimize/maximize-restore/close buttons for the Electron shell's frameless window (see [`../features/11-electron-desktop-app.md`](../features/11-electron-desktop-app.md)); renders nothing (`null`) outside Electron. Deliberately raw `<button>`s, not the shared `Button` — these mimic square, edge-to-edge, full-height OS titlebar chrome rather than app UI, so they intentionally don't follow the rounded/padded button convention. |

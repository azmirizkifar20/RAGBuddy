...
- **Ollama embedding resilience:** `OllamaEmbeddingProvider.embedOne` (`src/embedding/embedding-provider.ts`) retries a `5xx` response from Ollama's `/api/embeddings` up to twice with exponential backoff (500ms, 1s) before giving up — a local Ollama instance can return a transient `500` under load. A `4xx` is never retried (the request itself is wrong, not the server). `EMBEDDING_CONCURRENCY` (parallel in-flight embed calls for the Ollama provider only — the OpenAI-compatible provider batches all texts into one request) is `2`, kept low so a single local Ollama process isn't overloaded by concurrent calls for heavier models (e.g. `bge-m3`). See [../issue/2026-08-10_ollama-embedding-500.md](../issue/2026-08-10_ollama-embedding-500.md).
